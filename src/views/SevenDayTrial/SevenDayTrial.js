...
const formSchema = z.object({
  ...
  // If referralName is provided, referralCode must also be provided
  if (data.referralName && data.referralName.trim() !== '') {
    return data.referralCode && data.referralCode.trim() !== '';
  }
  return true;
}, {
  message: "Referral code is required when referral source is provided",
  path: ["referralCode"], // This will show the error on the referralCode field
});
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/freetrial/submit`,
        payload
      );

      if (response.data) {
        toast.dismiss();
        toast.success("Trial request submitted successfully! We'll contact you soon.");
        reset(); // Reset form
        setPhoneValue(""); // Reset phone input value
      }
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to submit trial request. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="flex justify-end">
              <div className="request-card custom-spacing">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  {errors.fullName && <p className="text-red-500 !text-xs text-start -mt-2 mb-2 pl-1">{errors.fullName.message}</p>}

                    <div className={`input-group ${errors.companyName ? "!border-red-500" : ""}`}>
                    <img
                  ...
                  {errors.companyName && <p className="text-red-500 !text-xs text-start -mt-2 mb-2 pl-1">{errors.companyName.message}</p>}
                  ...
                  {errors.email && <p className="text-red-500 !text-xs text-start -mt-2 mb-2 pl-1">{errors.email.message}</p>}
                  ...
                  {errors.phone && <p className="text-red-500 !text-xs text-start -mt-2 mb-2 pl-1">{errors.phone.message}</p>}
                  ...
                  {errors.profession && <p className="text-red-500 text-start !text-xs -mt-2 mb-2 pl-1">{errors.profession.message}</p>}
